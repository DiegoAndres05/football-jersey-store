...
import { FKA_IMPORTER_DISABLED_MESSAGE, isFkaImporterEnabled } from "../fka/browser-provider";
import {
  buildSeasonUrl,
  extractTeamContext,
  extractKitLinks,
...
export async function searchFkaPreviewAction(input: FkaSearchInput): Promise<FkaPreviewResult> {
  ...
  if (!isFkaImporterEnabled()) return { ok: false, error: FKA_IMPORTER_DISABLED_MESSAGE };
