...
export default async function AdminSuppliersPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <form action={updateAllSuppliersAction} className="rounded-xl border border-border bg-card overflow-hidden">
        ...
        {suppliers.map((s) => <input key={s.id} type="hidden" name="supplierId" value={s.id} />)}
                ...
                <td className="px-4 py-3">
                  <div className="flex flex-wrap justify-end gap-2">
                    <div className="flex flex-wrap items-center gap-2">
                      <input name={`supplier.${s.id}.name`} defaultValue={s.name} className="h-7 w-32 rounded-md border border-input bg-background px-2 text-xs" />
                      <input name={`supplier.${s.id}.contactName`} defaultValue={s.contactName ?? ""} placeholder="Contacto" className="h-7 w-24 rounded-md border border-input bg-background px-2 text-xs" />
                      <input name={`supplier.${s.id}.email`} defaultValue={s.email ?? ""} placeholder="Email" className="h-7 w-32 rounded-md border border-input bg-background px-2 text-xs" />
                      <input name={`supplier.${s.id}.phone`} defaultValue={s.phone ?? ""} placeholder="Tel" className="h-7 w-20 rounded-md border border-input bg-background px-2 text-xs" />
                      <input name={`supplier.${s.id}.country`} defaultValue={s.country ?? ""} placeholder="País" className="h-7 w-20 rounded-md border border-input bg-background px-2 text-xs" />
                      <input name={`supplier.${s.id}.leadTimeDays`} type="number" min={0} defaultValue={s.leadTimeDays} className="h-7 w-14 rounded-md border border-input bg-background px-2 text-xs" title="Lead días" />
                      <input name={`supplier.${s.id}.priority`} type="number" min={0} max={10} defaultValue={s.priority} className="h-7 w-12 rounded-md border border-input bg-background px-2 text-xs" title="Prioridad" />
                      <label className="flex items-center gap-1 text-xs text-muted-foreground">
                        <input type="checkbox" name={`supplier.${s.id}.isActive`} defaultChecked={s.isActive} className="accent-primary" /> Activo
                      </label>
                      <input name={`supplier.${s.id}.purchaseNotes`} defaultValue={s.purchaseNotes ?? ""} placeholder="Notas" className="hidden" />
                    </div>
                    <Link
