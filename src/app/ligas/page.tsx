          ...
          {leagues.map((league, i) => {
            ...
            return (
              <section key={league.slug} className="border-b border-border pb-10 last:border-b-0">
                ...
                {league.productCount === 0 ? (
                  <a
                    href={whatsappLink(`Hola Flashsport, quiero pedir la liga ${league.name}.`)}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="mt-4 inline-flex items-center gap-1.5 rounded-md border border-border bg-card px-3 py-1.5 text-sm font-medium hover:border-foreground hover:bg-accent transition-colors"
                  >
                    Pídela por encargo
